...
| `ESPCONTROL_IMAGE_CARD_MAX_CONTEXTS=2` | `guition-esp32-s3-4848s040` | Limits concurrent image-card contexts to two on the constrained S3 panel to preserve runtime memory headroom. | The full image-card context pool fits on the S3 panel with safe runtime heap headroom, or capacity is generated from device profile data. |
