...
export function companionEntityForMode(mode: string): string {
    ...
    if (mode === "window") return COMPANION_WINDOW_ACTIONS[0]?.id || "window.close";
                    ...
                    onChange: async function (this: HTMLSelectElement) {
                        ...
                        if ((previousMode === "app" || previousMode === "url") && previousEntity) {
                            try {
                                ...
                                previousGeneratedLabel = resolvedLabel;
                            } catch {
                                this.value = previousMode;
                                return;
                            }
                        } else if (previousMode === "window") {
                            previousGeneratedLabel = companionWindowActionLabel(previousEntity);
                        } else if (previousMode === "shortcut") {
                            previousGeneratedLabel = formatCompanionShortcutActionId(previousEntity);
                            ...
                            if (selectedOption && selectedOption.value === previousEntity) {
                        ...
                        if (previousGeneratedLabel && card.label === previousGeneratedLabel) card.label = "";
                        resetCompanionMediaPresentation(card, nextMode);
                        resetCompanionMetricPresentation(card, nextMode);
                        const selectedMetric = COMPANION_SYSTEM_METRICS.find((metric) => metric.mode === nextMode);
                        card.entity = companionEntityForMode(nextMode);
                        card.sensor = nextMode === "url" ? COMPANION_URL_PREFIX : "";
                        if (nextMode === "media") {
                            applyCompanionMediaPresentation(card, previousGeneratedLabel);
                        } else if (nextMode === "window" || nextMode === "shortcut") {
                            card.label = companionModeChangeLabel(
