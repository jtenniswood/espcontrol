...
The **System Stats** tab shares overall processor and memory usage, startup-disk storage, network throughput, and battery level when available. Sharing is enabled by default and can be disabled independently. System-statistic Companion cards show the corresponding reading and use `--` while unavailable.
...
The protocol is intentionally narrow: a Mac publishes approved bundle identifiers, opaque identifiers for approved folders, supported media controls, an optional Now Playing snapshot, and optional system statistics. The panel can request one of those applications or folders, a validated keyboard shortcut, a fixed allow-listed window action, media control, or a web URL opened by one of those applications. Artwork is resized into a black 480×480 JPEG and transferred in acknowledged authenticated chunks. It does not execute shell commands, accept app or folder paths from the panel, allow non-web URL schemes, or accept inbound network connections.
