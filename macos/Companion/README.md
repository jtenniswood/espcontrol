...
Now Playing and system statistics are shared automatically whenever the Companion is connected. Now Playing reads the macOS system media feed; system statistics include overall processor and memory usage, startup-disk storage, combined network throughput on the primary interface, and battery level when the Mac has a battery. Memory and storage cards can show either used or free capacity. These features do not require additional macOS permissions and do not collect application, file, browsing, or network-content details. Choose the corresponding type on a Companion card to show a reading.
...
The protocol is intentionally narrow: a Mac publishes installed bundle identifiers, opaque identifiers for user-approved folders, and system statistics when supported by the connected panel. The panel can request one of those applications or folders, a validated keyboard shortcut, media control, or a web URL opened by one of those applications. Folder paths stay in the Mac app and are never sent to the display. It does not execute shell commands, accept app or folder paths from the panel, allow non-web URL schemes, or accept inbound network connections.
