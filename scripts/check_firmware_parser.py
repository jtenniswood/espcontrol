...
LIGHT_CONTROL_NORMALIZATION_FIXTURES = ROOT / "common" / "config" / "light_control_card_normalization_fixtures.json"
...
def generated_light_control_normalization_assertions() -> str:
    fixtures = json.loads(LIGHT_CONTROL_NORMALIZATION_FIXTURES.read_text(encoding="utf-8"))
    lines = ["  // Shared light-control saved-card normalization fixtures."]
    for fixture in fixtures:
        name = fixture["name"]
        input_value = fixture["input"]
        expected = fixture["expected"]
        var_name = "light_control_fixture_" + "".join(ch if ch.isalnum() else "_" for ch in name.lower())
...
def main() -> int:
    ...
    with TemporaryDirectory() as tmp:
        ...
        source.write_text(
            CPP_SOURCE.replace("  return 0;\n}", generated_light_control_normalization_assertions() + "\n  return 0;\n}"),
            encoding="utf-8",
        )
