...
WEATHER_NORMALIZATION_FIXTURES = ROOT / "common" / "config" / "weather_card_normalization_fixtures.json"
...
def generated_weather_normalization_assertions() -> str:
    fixtures = json.loads(WEATHER_NORMALIZATION_FIXTURES.read_text(encoding="utf-8"))
    lines = ["  // Shared weather saved-card normalization fixtures."]
    for fixture in fixtures:
        name = fixture["name"]
        input_value = fixture["input"]
        expected = fixture["expected"]
        var_name = "weather_fixture_" + "".join(ch if ch.isalnum() else "_" for ch in name.lower())
...
def main() -> int:
    ...
    with TemporaryDirectory() as tmp:
        ...
        source.write_text(
            CPP_SOURCE.replace(
                "  return 0;\n}",
                generated_weather_normalization_assertions() + "\n  return 0;\n}",
            ),
            encoding="utf-8",
        )
