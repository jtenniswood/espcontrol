...
def test_trmnl_epaper_card_text_sizing() -> None:
    ...
    assert btn["valueSize"] >= 18.4, "TRMNL web preview sensor values regressed to smaller text"
    assert btn["labelSize"] >= 9.9, "TRMNL web preview card labels regressed to smaller text"

    tile = (ROOT / "devices" / "trmnl-75-og" / "device" / "trmnl_tile_widget.yaml").read_text(
        encoding="utf-8"
    )
    assert "id: trmnl_tile_${num}_value" in tile and "transform_scale: 3.1" in tile, (
        "TRMNL physical sensor value text regressed to a smaller scale"
    )
    assert "id: trmnl_tile_${num}_label" in tile and "transform_scale: 3.3" in tile, (
