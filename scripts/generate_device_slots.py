...
def card_background_image_package_text(slot_count: int) -> str:
    lines = [
        "# =============================================================================",
        f"# CARD BACKGROUND IMAGE DECODERS - {slot_count}-slot runtime pool",
        "# Generated by scripts/generate_device_slots.py from devices/manifest.json.",
        "# =============================================================================",
        "",
        "artwork_image:",
    ]
    for num in range(1, slot_count + 1):
        lines.extend(
            [
                '  - url: "http://127.0.0.1/unused-card-image.jpg"',
                f"    id: card_background_download_{num}",
                "    format: JPEG",
                "    type: RGB565",
    ...
    lines.extend(
        [
            ...
            *[
            ...
            include_line(
                "card_bg_images",
                f"!include ../../common/device/card_background_images_{device['card_background_resident_frames']}.yaml",
            ),
            *(
                [
                    include_line(
...
def cfg_lines(device: dict) -> list[str]:
    ...
    card_background_count = device["card_background_resident_frames"]
    ...
    lines.append('                          "Card background resident frame budget exceeds this device grid");')
...
def main() -> int:
    ...
    changed: list[Path] = []
    for slot_count in sorted({int(device["card_background_resident_frames"]) for device in slot_devices()}):
        package_path = ROOT / "common" / "device" / f"card_background_images_{slot_count}.yaml"
        update(package_path, card_background_image_package_text(slot_count), args.check, changed)
