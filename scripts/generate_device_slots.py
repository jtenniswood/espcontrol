...
def partition_filename(device: dict) -> str:
    flash_mb = int(device["flash_bytes"]) // (1024 * 1024)
    return f"partitions_{flash_mb}mb_card_images.csv"


def card_asset_capabilities_text(device: dict) -> str:
    return "\n".join(
        [
            "# =============================================================================",
            "# CARD ASSET CAPABILITIES",
            "# Generated by scripts/generate_device_slots.py from devices/catalog.json.",
            "# =============================================================================",
            "",
            "esphome:",
            "  build_flags:",
            f'    - "-DESPCONTROL_MAX_GRID_SLOTS={int(device["slots"])}"',
            f'    - "-DESPCONTROL_CARD_BACKGROUND_MAX_ACTIVE={int(device["card_background_max_active"])}"',
            f'    - "-DESPCONTROL_CARD_BACKGROUND_RESIDENT_FRAMES={int(device["card_background_resident_frames"])}"',
            f'    - "-DESPCONTROL_CARD_IMAGE_PARTITION_BYTES={int(device["card_background_partition_bytes"])}"',
            "",
        ]
    )


def card_image_partition_text(flash_bytes: int, partition_bytes: int) -> str:
    # Card and subpage configuration values are stored as ESPHome preferences.
    # The former 20 KiB NVS partition could fill up once longer card-image IDs
    # were added to existing configurations, causing changes to disappear after
    # reboot. Keep ample headroom for settings churn and future config fields.
    app_start = 0x20000
    card_start = flash_bytes - partition_bytes
    app_size = ((card_start - app_start) // 2) & ~0xFFFF
    app1_start = app_start + app_size
    card_offset = f"0x{card_start:x},"
    return "\n".join(
        [
            "# Name,       Type, SubType, Offset,    Size,      Flags",
            "#",
            f"# This is the deployed layout for {flash_bytes // (1024 * 1024)} MB EspControl panels.  Keep it explicit:",
            ...
            f"app0,          app,  ota_0,   0x{app_start:x},   0x{app_size:x},",
            f"app1,          app,  ota_1,   0x{app1_start:x},  0x{app_size:x},",
            f"card_images,   data, 0x40,    {card_offset:<11}0x{partition_bytes:x},",
            "",
        ]
    )
    ...
    lines.extend(
        [
            ...
            *package_substitution_lines(device),
            ...
            include_line("card_asset_capabilities", "!include device/card_asset_capabilities.yaml"),
        ]
    )
...
def cfg_lines(device: dict) -> list[str]:
    ...
    lines.append("            static_assert(sizeof(card_background_downloaders) / sizeof(card_background_downloaders[0]) == CARD_BACKGROUND_RESIDENT_FRAMES,")
    lines.append('                          "Generated card background pool differs from the device capability");')
...
def main() -> int:
    ...
    partition_layouts: dict[str, tuple[int, int]] = {}
    for device in devices:
        filename = partition_filename(device)
        layout = (int(device["flash_bytes"]), int(device["card_background_partition_bytes"]))
        if filename in partition_layouts and partition_layouts[filename] != layout:
            print(f"ERROR: conflicting generated partition layout for {filename}", file=sys.stderr)
            return 1
        partition_layouts[filename] = layout
    for filename, (flash_bytes, partition_bytes) in sorted(partition_layouts.items()):
        path = ROOT / "common" / "device" / filename
        update(path, card_image_partition_text(flash_bytes, partition_bytes), args.check, changed)

    for slot_count in sorted({int(device["card_background_resident_frames"]) for device in devices}):
    ...
    for device in devices:
        ...
        capability_path = ROOT / "devices" / slug / "device" / "card_asset_capabilities.yaml"
        try:
        ...
        update(capability_path, card_asset_capabilities_text(device), args.check, changed)
