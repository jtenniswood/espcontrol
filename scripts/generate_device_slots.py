...
def package_substitution_lines(device: dict) -> list[str]:
    ...
    lines = [
        ...
        # Bounds and defaults for the user-configurable main grid, consumed by
        # common/config/grid_dimensions.yaml.
        f'  grid_default_cols: "{device["cols"]}"',
        f'  grid_default_rows: "{device["rows"]}"',
        f'  grid_max_cols: "{device["max_cols"]}"',
        f'  grid_max_rows: "{device["max_rows"]}"',
    ]
...
def package_file_text(device: dict) -> str:
    ...
    lines.extend(
        [
            ...
            include_line("grid_dimensions", "!include ../../common/config/grid_dimensions.yaml"),
...
def button_package_block(device: dict) -> str:
    ...
    lines = [
        ...
        f"  # Per-button entity/label/icon ({device['max_slots']} compiled slots - "
        f"default {device['grid']} grid, runtime-configurable up to "
        f"{device['max_cols']}x{device['max_rows']})",
    ]
    for num in range(1, device["max_slots"] + 1):
...
def cfg_lines(device: dict) -> list[str]:
    ...
    max_slots = int(device["max_slots"])
    default_cols = int(device["cols"])
    default_rows = int(device["rows"])
    # The active grid is read at runtime from the user-configurable number
    # entities. Each dimension is bounded to [1..maxCols]/[1..maxRows] by the
    # entity, so the product can never exceed the compiled max_slots array size;
    # the clamp below is defensive only.
    lines = [
        "            GridConfig cfg = {};",
        "            int grid_cols_setting = (int) (id(grid_cols_number).state + 0.5f);",
        "            int grid_rows_setting = (int) (id(grid_rows_number).state + 0.5f);",
        f"            if (grid_cols_setting < 1) grid_cols_setting = {default_cols};",
        f"            if (grid_rows_setting < 1) grid_rows_setting = {default_rows};",
        f"            if (grid_cols_setting * grid_rows_setting > {max_slots}) {{",
        f"              grid_cols_setting = {default_cols};",
        f"              grid_rows_setting = {default_rows};",
        "            }",
        "            cfg.num_slots = grid_cols_setting * grid_rows_setting;",
        f"            cfg.compiled_slots = {max_slots};",
    ]
    if "portrait_cols" in device:
        ...
        # In portrait the display is rotated 90/270, so the user\'s column and
        # row choices swap to keep the on-screen layout intuitive.
        lines.append("            cfg.cols = portrait ? grid_rows_setting : grid_cols_setting;")
        # Default grid in the active orientation, so icons render full-size at
        # the default and scale down as the user makes the grid denser.
        lines.append(f"            cfg.base_cols = portrait ? {default_rows} : {default_cols};")
        lines.append(f"            cfg.base_rows = portrait ? {default_cols} : {default_rows};")
        if device.get("rotate_width_compensation", False):
            lines.append("            cfg.width_compensation_vertical = portrait;")
    else:
        lines.append("            cfg.cols = grid_cols_setting;")
        lines.append(f"            cfg.base_cols = {default_cols};")
        lines.append(f"            cfg.base_rows = {default_rows};")
...
def refresh_subpage_arrays(device: dict) -> list[str]:
    ...
    lines.extend(macro_array("sp_cfgs", "SP_CFG", device["max_slots"], indent=indent))
    lines.extend(macro_array("sp_ext", "SP_EXT", device["max_slots"], indent=indent))
    lines.extend(macro_array("sp_ext2", "SP_EXT2", device["max_slots"], indent=indent))
    lines.extend(macro_array("sp_ext3", "SP_EXT3", device["max_slots"], indent=indent))
    if subpage_chunks >= 8:
        lines.extend(macro_array("sp_ext4", "SP_EXT4", device["max_slots"], indent=indent))
        lines.extend(macro_array("sp_ext5", "SP_EXT5", device["max_slots"], indent=indent))
        lines.extend(macro_array("sp_ext6", "SP_EXT6", device["max_slots"], indent=indent))
        lines.extend(macro_array("sp_ext7", "SP_EXT7", device["max_slots"], indent=indent))
    lines.extend(
        [
            "          #undef SP_CFG",
            "          #undef SP_EXT",
...
def phase2_block(device: dict) -> str:
    ...
    for num in range(1, device["max_slots"] + 1):
        lines.append(f"              BTN_SLOT({num}),")
    lines.append("            };")
    lines.extend(macro_array("sp_cfgs", "SP_CFG", device["max_slots"]))
    lines.extend(macro_array("sp_ext", "SP_EXT", device["max_slots"]))
    lines.extend(macro_array("sp_ext2", "SP_EXT2", device["max_slots"]))
    lines.extend(macro_array("sp_ext3", "SP_EXT3", device["max_slots"]))
    if subpage_chunks >= 8:
        lines.extend(macro_array("sp_ext4", "SP_EXT4", device["max_slots"]))
        lines.extend(macro_array("sp_ext5", "SP_EXT5", device["max_slots"]))
        lines.extend(macro_array("sp_ext6", "SP_EXT6", device["max_slots"]))
        lines.extend(macro_array("sp_ext7", "SP_EXT7", device["max_slots"]))
    lines.extend(
        [
            "            #undef BTN_SLOT",
            "            #undef SP_CFG",
