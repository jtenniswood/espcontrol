...
def saved_config_declarative_field_lines(field, rule, language):
    ...
    if policy == "keep":
        return []
    if policy == "declared_options":
        # Keep only the options this card declares, in canonical order. That is
        # what "unknownOptions": "drop" promises, expressed declaratively so a
        # static card can carry options without a bespoke hook.
        names = json.dumps(rule["names"], ensure_ascii=False)
        if language == "ts":
            return [f"    {target} = filterDeclaredOptions({target}, {names});\n"]
        cpp_names = ", ".join(json.dumps(name) for name in rule["names"])
        return [
            f"    {target} = filter_declared_options({target}, {{{cpp_names}}});\n"
        ]
    if policy == "clear":
