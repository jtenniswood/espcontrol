...
WEB_FIXED_MDI_ICON_CODEPOINTS = {
    # "alert" and "information" are notification level names rather than icons
    # the editor draws, but they are also real MDI glyph names, so the source
    # scan below finds them and expects a codepoint either way.
    "alarm": "F0020", "album": "F0025", "alert": "F0026", "api": "F109B",
    "arrow-expand-all": "F004C", "information": "F02FC",
...
SAVED_CONFIG_NOTIFICATION_TS = ROOT / "src" / "webserver" / "generated" / "saved_config_notification.ts"
SAVED_CONFIG_NOTIFICATION_H = ROOT / "components" / "espcontrol" / "button_grid_saved_config_notification_generated.h"
...
def saved_config_notification_normalization(data):
    normalization = data["cards"]["notification"].get("normalization")
    if not normalization:
        raise BuildError("notification saved-config card requires authored normalization")
    field_hooks = {
        rule.get("hook") for field, rule in normalization["fields"].items()
        if field != "options" and rule.get("policy") == "hook"
    }
    if field_hooks:
        raise BuildError("notification saved-config fields must stay declarative")
    option_rule = normalization["fields"]["options"]
    if option_rule.get("policy") != "hook" or option_rule.get("hook") != "normalize_notification_options":
        raise BuildError("notification saved-config card requires the authored option hook")
    if normalization.get("optionHook") != "normalize_notification_options":
        raise BuildError("notification saved-config card has an unexpected optionHook")
    return normalization


def gen_saved_config_notification_ts(data):
    normalization = saved_config_notification_normalization(data)
    lines = [
        "// =============================================================================\n",
...
def gen_saved_config_notification_h(data):
    normalization = saved_config_notification_normalization(data)
    lines = [
        "#pragma once\n\n",
        "// =============================================================================\n",
...
def gen_card_contract_h(data):
    ...
    lines = [
        ...
        f'constexpr const char *CARD_CONTRACT_NOTIFICATION_LEVEL_ATTRIBUTE_DEFAULT = {json.dumps(contract_card_option_default(cards, "notification", "level_attribute"))};\n',
...
def sync_card_contract(check_only=False):
    ...
    outputs = [
        ...
        (SAVED_CONFIG_NOTIFICATION_TS, gen_saved_config_notification_ts(data)),
        (SAVED_CONFIG_NOTIFICATION_H, gen_saved_config_notification_h(data)),
