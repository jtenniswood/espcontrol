...
inline void pin_keypad_update_display() {
  ...
  if (ui.pin.empty()) {
    lv_label_set_text(ui.pin_lbl, espcontrol_i18n(
      ui.empty_label ? ui.empty_label : "Enter Pin"));
    return;
  }
...
inline void pin_keypad_key_cb(lv_event_t *event) {
  const std::string *key = static_cast<const std::string *>(
...
inline lv_obj_t *pin_keypad_create_key_button(
    lv_obj_t *parent,
    lv_coord_t width,
    lv_coord_t height,
    const char *text,
  ...
  lv_obj_t *button = control_modal_create_round_button(
...
inline lv_coord_t pin_keypad_label_y(const ControlModalLayout &layout,
                                     lv_coord_t pin_h) {
...
inline bool pin_keypad_open_modal(
  ...
  ControlModalShell shell = control_modal_open_shell(
    pin_keypad_modal_kind(kind), source_btn, width_compensation_percent,
    icon_font, pin_keypad_hide_modal);
  if (!shell.overlay || !shell.panel) return false;
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (key == "submit") {
      ...
      if (key_lbl) {
        lv_obj_set_style_text_color(key_lbl, lv_color_hex(DARK_TEXT_PRIMARY), LV_PART_MAIN);
      }
    }
    int row = i / 3;
    int col = i % 3;
    ...
    lv_obj_add_event_cb(key_btn, pin_keypad_key_cb, LV_EVENT_CLICKED,
                        &ui.key_data[i]);
  }
...
inline void screensaver_pin_cancel_reopen() {
  ScreensaverPinKeypadOpenArgs &args = screensaver_pin_keypad_open_args();
  if (!args.reopen_timer) return;
  lv_timer_del(args.reopen_timer);
  args.reopen_timer = nullptr;
}

inline bool screensaver_pin_keypad_submit(const std::string &code, void *) {
  if (normalize_numeric_pin(code) != screensaver_pin_unlock_code()) return false;
  screensaver_pin_cancel_reopen();
...
inline void screensaver_pin_reopen_timer_cb(lv_timer_t *timer) {
  ...
  pin_keypad_open_modal(
    ...
    screensaver_pin_keypad_submit, nullptr, screensaver_pin_reopen_if_locked);
}
...
inline void screensaver_pin_clear_lock() {
  screensaver_pin_cancel_reopen();
  screensaver_pin_locked() = false;
  screensaver_pin_unlock_code().clear();
...
inline bool screensaver_pin_lock_and_open(
  ...
  screen_lock_apply();
  bool opened = pin_keypad_open_modal(
    PinKeypadKind::SCREENSAVER, source_btn, width_compensation_percent,
    pin_label_font, key_label_font, icon_font, "Enter Pin",
    screensaver_pin_keypad_submit, nullptr, screensaver_pin_reopen_if_locked);
  if (!opened) {
    screensaver_pin_clear_lock();
    return false;
