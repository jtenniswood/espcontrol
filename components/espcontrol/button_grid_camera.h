...
//
// Shared camera/image attachment engine. The existing Camera Card adapter keeps
// the public `image` card behavior stable while future card types can opt in to
// a camera background and shared full-screen modal.
...
constexpr const char *CAMERA_ATTACHMENT_ENTITY_OPTION = "camera_entity";
constexpr const char *CAMERA_ATTACHMENT_MODAL_MODE_OPTION = "camera_modal_mode";
constexpr const char *CAMERA_ATTACHMENT_TAP_OPTION = "camera_tap";

enum class CameraAttachmentTapMode {
  ACTION,
  MODAL,
};
...
using CameraAttachmentCtx = ImageCardCtx;

inline std::string camera_attachment_entity_from_options(const std::string &options) {
  return cfg_option_value(options, CAMERA_ATTACHMENT_ENTITY_OPTION);
}

inline bool camera_attachment_modal_fit_from_options(const std::string &options) {
  return cfg_option_value(options, CAMERA_ATTACHMENT_MODAL_MODE_OPTION) == "fit";
}

inline CameraAttachmentTapMode camera_attachment_tap_mode_from_options(
  const std::string &options) {
  return cfg_option_value(options, CAMERA_ATTACHMENT_TAP_OPTION) == "modal"
    ? CameraAttachmentTapMode::MODAL
    : CameraAttachmentTapMode::ACTION;
}

inline CameraAttachmentConfig camera_attachment_config_from_options(
  const std::string &options,
  CameraAttachmentTapMode default_tap_mode = CameraAttachmentTapMode::ACTION) {
  CameraAttachmentConfig camera;
  camera.entity_id = camera_attachment_entity_from_options(options);
  camera.modal_fit = camera_attachment_modal_fit_from_options(options);
  camera.tap_mode = cfg_option_value(options, CAMERA_ATTACHMENT_TAP_OPTION).empty()
    ? default_tap_mode
    : camera_attachment_tap_mode_from_options(options);
  return camera;
}
...
inline lv_obj_t *setup_camera_attachment_background(BtnSlot &s) {
  ...
  if (loading) lv_obj_move_background(loading);
  return img;
}
...
inline bool bind_camera_attachment(BtnSlot &s, lv_obj_t *widget,
                                   const CameraAttachmentConfig &camera,
  ...
  if (!image_card_entity_supported(camera.entity_id)) {
    ESP_LOGW("camera_attachment", "Camera attachment only supports camera and image entities: %s",
             camera.entity_id.c_str());
    image_card_set_loading_state(loading, "Unavailable");
    return false;
  }
  ImageCardCtx *ctx = acquire_image_card_context(cfg);
  if (!ctx) {
    ESP_LOGW("camera_attachment", "No image downloader available for %s",
             camera.entity_id.c_str());
  ...
  ctx->entity_id = camera.entity_id;
  ...
  const std::string image_card_entity_id = camera.entity_id;
  ...
  subscribe_image_card_entity_state(ctx, camera.entity_id);
  image_card_request_picture(ctx);
  return true;
}

inline bool bind_camera_attachment_from_options(
  BtnSlot &s, const ParsedCfg &p, const GridConfig &cfg,
  CameraAttachmentTapMode default_tap_mode = CameraAttachmentTapMode::ACTION) {
  CameraAttachmentConfig camera = camera_attachment_config_from_options(
    p.options, default_tap_mode);
  if (camera.entity_id.empty()) return false;
  lv_obj_t *widget = setup_camera_attachment_background(s);
  return bind_camera_attachment(s, widget, camera, cfg);
}
...
inline bool bind_image_card(BtnSlot &s, const ParsedCfg &p, const GridConfig &cfg,
  ...
  CameraAttachmentConfig camera;
  camera.entity_id = p.entity;
  camera.tap_mode = bind_click_handler
    ? CameraAttachmentTapMode::MODAL
    : CameraAttachmentTapMode::ACTION;
