...
// A live rebuild holds the old graph until the replacement is complete, so
// channel storage must accommodate two disjoint generations at once.
constexpr size_t HA_CONFIGURED_CARD_CAPACITY =
    static_cast<size_t>(MAX_GRID_SLOTS + MAX_SUBPAGE_ITEMS);
constexpr size_t HA_STATE_CHANNEL_CAPACITY_PER_GENERATION =
