...
void card_background_runtime_shutdown(CardBackgroundRuntimeState &runtime) {
...
CardBackgroundRuntimeState &card_background_runtime() {
...
CardBackgroundImageCtx *card_background_image_contexts() {
  return card_background_runtime().contexts;
}

CardBackgroundController &card_background_controller() {
  return card_background_runtime().controller;
}

int card_background_context_index(const CardBackgroundImageCtx *ctx) {
  if (!ctx) return CardBackgroundController::NO_RESOURCE;
  return static_cast<int>(ctx - card_background_image_contexts());
}

espcontrol::card_background::ResourceState &card_background_state(
    CardBackgroundImageCtx *ctx) {
  return card_background_controller().resource(
    static_cast<size_t>(card_background_context_index(ctx)));
}

const espcontrol::card_background::ResourceState &card_background_state(
    const CardBackgroundImageCtx *ctx) {
  return card_background_controller().resource(
    static_cast<size_t>(card_background_context_index(ctx)));
}

CardBackgroundImageCtx *card_background_context_at(int index) {
  if (index < 0 || index >= CARD_BACKGROUND_IMAGE_MAX_CONTEXTS) return nullptr;
  return &card_background_image_contexts()[index];
}

std::vector<CardBackgroundWidgetRef> &card_background_widget_refs() {
  return card_background_runtime().widget_refs;
}

lv_obj_t *&card_background_active_page() {
  return card_background_runtime().active_page;
}

lv_timer_t *&card_background_final_refresh_timer() {
  return card_background_runtime().final_refresh_timer;
}

lv_timer_t *&card_background_cache_write_timer() {
  return card_background_runtime().cache_write_timer;
}

lv_timer_t *&card_background_next_download_timer() {
  return card_background_runtime().next_download_timer;
}

bool card_background_widget_on_page(lv_obj_t *btn, lv_obj_t *page);

void card_background_sync_label_shadow_text(lv_obj_t *label,
...
void move_content_foreground(const BtnSlot &s) {
...
bool configured_for_card(const ParsedCfg &p) {
...
void card_background_position_widget(lv_obj_t *btn, lv_obj_t *widget) {
  lv_coord_t width = 0;
  lv_coord_t height = 0;
...
CardBackgroundImageCtx *card_background_active_download_context() {
  return card_background_context_at(card_background_controller().active_download());
}

void card_background_set_widget_source_hidden(
...
void card_background_release_download_slot(CardBackgroundImageCtx *ctx);
bool card_background_reveal_ready_widgets();
void card_background_handle_download_error(CardBackgroundImageCtx *ctx);
void card_background_schedule_cache_writes();
void card_background_schedule_next_download(CardBackgroundImageCtx *finished_ctx);

void card_background_request_download(CardBackgroundImageCtx *ctx) {
...
bool card_background_start_next_queued_download(CardBackgroundImageCtx *finished_ctx) {
...
void card_background_apply_downloaded(CardBackgroundImageCtx *ctx, bool cached) {
...
void card_background_bind_callbacks(CardBackgroundImageCtx *ctx) {
...
std::string card_background_image_url(const std::string &id);

void card_background_configure_target_size(CardBackgroundImageCtx *ctx,
...
bool card_background_context_has_active_bindings(CardBackgroundImageCtx *ctx) {
  return ctx && card_background_controller().has_bindings(
    static_cast<size_t>(card_background_context_index(ctx)));
}

void card_background_release_contexts(const GridConfig &cfg) {
  lv_timer_t *&refresh_timer = card_background_final_refresh_timer();
  if (refresh_timer) {
    lv_timer_del(refresh_timer);
    refresh_timer = nullptr;
...
void reset_image_pool(const GridConfig &cfg) {
  card_background_release_contexts(cfg);
  card_background_active_page() = nullptr;
  card_background_widget_refs().clear();
}

CardBackgroundImageCtx *acquire_card_background_image_context(const GridConfig &cfg,
                                                                     const std::string &id,
                                                                     int target_width,
                                                                     int target_height);

CardBackgroundImageCtx::Binding *card_background_add_binding(CardBackgroundImageCtx *ctx,
                                                                    lv_obj_t *btn,
                                                                    lv_obj_t *widget);

void card_background_sync_binding_image(CardBackgroundImageCtx *ctx,
...
void card_background_deactivate_if_unused(CardBackgroundImageCtx *ctx) {
...
bool card_background_move_binding_to_size(CardBackgroundImageCtx *ctx,
                                                 CardBackgroundImageCtx::Binding &binding,
...
void refresh_card_background_image(lv_obj_t *btn, const GridConfig &cfg) {
  if (!btn) return;
  CardBackgroundImageCtx *contexts = card_background_image_contexts();
  for (int i = 0; i < CARD_BACKGROUND_IMAGE_MAX_CONTEXTS; i++) {
    CardBackgroundImageCtx *ctx = &contexts[i];
...
void card_background_register_widget(lv_obj_t *btn, lv_obj_t *widget,
                                            const std::string &id) {
...
void card_background_unregister_widget(lv_obj_t *widget) {
  if (!widget) return;
...
void activate_page(const GridConfig &cfg, lv_obj_t *page) {
  if (!page) return;
  int total_refs = 0;
  int matched_refs = 0;
...
void apply_image(BtnSlot &s, const ParsedCfg &p,
                                        const GridConfig &cfg) {
...
void sync_image(BtnSlot &s, const ParsedCfg &p,
                                       const GridConfig &cfg) {
  if (!s.btn) return;
  ...
  if (existing_widget && existing_id == desired_id) {
    ...
    if (!desired_id.empty()) move_content_foreground(s);
    return;
  }
  ...
  if (!desired_id.empty()) apply_image(s, p, cfg);
}

void clear_image(BtnSlot &s) {
  if (!s.btn) return;
  lv_obj_t *existing_widget = nullptr;
  for (const auto &ref : card_background_widget_refs()) {
    if (ref.btn == s.btn) {
