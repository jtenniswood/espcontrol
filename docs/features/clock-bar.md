...
Tap the network status icon on the panel to open Settings. The page keeps the clock bar visible, shows **Settings** to the left of the clock, and uses the device's normal grid—for example, 5×3 on a 7-inch display. Back, IP address, Backlight, and Build flow through that grid from left to right and then top to bottom. **IP address** shows the current address, **Build** shows the installed firmware version, and the backlight slider changes the saved Manual brightness or the active Daytime/Nighttime level without changing brightness mode. Tap **Back** to return to the page you were using and restore its temperature display.
