...
Tap the round button in the bottom-right corner to pause the displayed player. The artwork stays visible and the button changes to Play, so you can resume without leaving the screensaver. The button stays visible when track details fade away. It uses the artwork's extracted colour and lightens while pressed, with a neutral grey fallback when artwork is unavailable.
