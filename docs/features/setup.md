...
Open **Settings > System > Home Assistant Settings** to manage camera/image and media artwork connections. **Automatic** prefers the complete local URL advertised by the connected Home Assistant instance, keeping its protocol, hostname and port together. Home Assistant's separate Internet URL is not used.
...
Choose **Manual** to set **Home Assistant Protocol** (`http` or `https`) and **Home Assistant Port** (1–65535). Manual mode keeps using the IP of the connected Home Assistant API client and never changes protocol automatically. It does not provide a hostname or full-URL override. A proxy that requires a hostname needs a reachable advertised local URL; if that cannot be discovered, this connection layout may require a future full-URL override.
