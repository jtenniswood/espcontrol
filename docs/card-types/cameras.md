...
On ESP32-P4 panels, the same `camera.*` and `image.*` entities can also be selected as the **Camera** screensaver action in **Settings > Sleep & Schedule > Screensaver**. That full-screen view is separate from the Camera-card pool, initially uses **Show full image** framing, and does not consume a Camera-card slot.
:::
