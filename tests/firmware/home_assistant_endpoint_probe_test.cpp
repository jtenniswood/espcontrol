...
int main() {
 ...
 assert(probe.start("https://ha.example:8443", 1));
 assert(!probe.take(result));
 assert(!probe.start("http://10.1.2.3:80", 2));
 probe_test_run_task();
 ...
 assert(probe.start("https://10.1.2.3:8123", 2));
 ...
 assert(probe.start("https://[fd12:0:0:0:0:0:0:1]:8443", 3));
